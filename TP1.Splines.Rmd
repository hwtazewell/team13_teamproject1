...
```
|  To fit the regression splines in R, we use library(splines). Section 7.4 exhibited that regression splines can be fitted through an appropriate matrix of basis functions. The bs() function generates the entire matrix of basis functions for splines using the specified set of knots. The default for bs() is to produce cubic splines. Below, wage has been fitted to age using a regression spline. 
...
|   In our first call to smooth.spline(), we specified that df=16. With this input information, the function then determines which value of lambda leads to 16 degrees of freedom. The second call to smooth.spline(), we select the smoothness level through crossvalidation (shown as cv = TRUE); the result of this is a  lambda that yields 6.8 degrees of freedom. In order to perform local regression, we use the loess() function. Because both models are similar in fit, we would select the smoothing spline model with the lower degrees of freedom (6.8). In other words, we chose the simpler model. 
